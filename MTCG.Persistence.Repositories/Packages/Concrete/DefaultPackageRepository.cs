...
namespace MTCG.Persistence.Repositories.Packages.Concrete;
    ...
    public async ValueTask<Package?> GetFirstNotAcquiredPackage()
    {
        try
        {
            ...
            IEnumerable<Card> cards = await connection.QueryAsync<Card>(
                "SELECT packagecardid as usercardid, cards.cardid, name, damage " +
                "FROM packagecontents " +
                "INNER JOIN cards ON packagecontents.cardid = cards.cardid " +
                "WHERE packagecontents.packageid = @packageid",
